...
class IT85xxPlus:
    ...
    def it85_cc_setup(self, max_curr, max_power, use_4wire=False, volt_auto=True, max_volt=30):
        ...
        self.it85_setup('SOUR:CURR:PROT 0')
    ...
    def it85_get_settings(self):
        self.it85_info_dict = {
            ...
            'Prog Load Current Protect': bool(self.it85_query('SOUR:CURR:PROT:STAT?')),
    ...
    def it85_channel_on_off(self, switch=False):
        self.it85_setup('SOUR:INP {}'.format(int(switch)))
        redo = 5
        for i in range(redo):
            read = int(self.it85_query('SOUR:INP?'))
            if read == int(switch):
                print('Prog Load is {}'.format(['OFF', 'ON'][read]))
                return
        print('FAIL to Control Input On Off!!!')

    def it85_cc_current(self, cc=0.0):
        self.it85_setup('SOUR:CURR:LEV {}'.format(float(cc)))
        redo = 5
        for i in range(redo):
            read = float(self.it85_query('SOUR:CURR:LEV?'))
            if read == float(cc):
                print('Prog Load CC is set to {} (A)'.format(read))
                return read
        print('FAIL setup CC-Mode Current Settings!!!')
        return None
